...
#    Essa função carrega todas as imagens de um diretório específico. Suporta arquivos PNG, JPG e JPEG.

def load_images(folder):    
    return [pygame.image.load(os.path.join(folder, f)) for f in os.listdir(folder) if f.endswith(('png', 'jpg', 'jpeg'))]

#    Essa função cria as peças do quebra-cabeça redimensionando as imagens carregadas.

def create_tiles(images, rows, cols):
    return [pygame.transform.scale(img, (TILE_SIZE, TILE_SIZE)) for img in images[:rows * cols]]

#    Essa função inicializa o jogo com o nível selecionado. Coloca as peças do quebra-cabeça em colunas e suas posições iniciais.
...
#    Essa função exibe a tela de seleção de nível com 3 botões clicáveis e vai para o nível selecionado pelo usuário.
...
#    Essa função exibe a tela inicial com um título e dois botões: "Iniciar Jogo" e "Sair". Se o usuário clicar em "Iniciar Jogo" vai para a tela de seleção de niveis, e encerra o jogo caso clique em "Sair" 
...
#    Essa é a função principal que gerencia o fluxo do jogo. Inicia a tela de seleção de nível e o jogo propriamente dito.
...
def main():    
    ...
    while True:
        ...
        complete_button = pygame.Rect(SCREEN_WIDTH - 150, 20, 130, 50)  #    Move o botão para o canto superior direito
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if complete_button.collidepoint(event.pos):
                        running = False  #    Volta para a tela inicial
